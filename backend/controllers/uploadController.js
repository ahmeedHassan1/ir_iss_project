...
export const uploadDocuments = async (req, res) => {
	try {
		...
		for (const file of files) {
         ...
         (doc_id, filename, encrypted_content, iv, auth_tag, content_hash, word_count, uploaded_by, is_encrypted) 
...
export const getDocument = async (req, res) => {
	try {
       ...
       WHERE d.doc_id = $1`,
		...
		let content = null;
		if (doc.encrypted_content && doc.iv && doc.auth_tag) {
			try {
...
export const deleteDocument = async (req, res) => {
	try {
		...
		await logAction(req, "DELETE", "FAILED", {
			docId: req.params.id,
			error: error.message
		});
		res.status(500).json({
