 ...
 */
export const findMatchingDocuments = async (includeTerms, excludeTerms = []) => {
  try {
...
export const calculateCosineSimilarity = (queryVector, docVector) => {
  ...
  const allTerms = new Set([...Object.keys(queryVector), ...Object.keys(docVector)]);
...
export const processQuery = async (queryString) => {
  try {
    ...
    const matchingDocIds = await findMatchingDocuments(includeTerms, excludeTerms);
    ...
    for (const docId of matchingDocIds) {
      ...
      const docResult = await query(
        'SELECT doc_id, filename, content, word_count FROM documents WHERE doc_id = $1',
        [docId]
      );
      ...
      if (docResult.rows.length > 0) {
        ...
        const snippet = generateSnippet(doc.content, includeTerms, 200);
